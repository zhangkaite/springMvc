...
/**
*
* 验证码生成器，使用此类需要将表单里的验证码输入框的name属性设为""
*/
public class VerifyCodeGenerator {

    private final String ATTRIBUTE_NAME = "rand";
    //图片的宽度
    ...
    private BufferedImage getImage(String verifyCode){
        ...
        //画边框
        for (int i=0; i<2; i++)
            graphics.drawRect(i, i, WIDTH  - i * 2 - 1, HEIGHT - i * 2 - 1);

        //设置随机干扰线条颜色
        graphics.setColor(getRandColor(50,100));
        //产生50条干扰线条
        for (int i=0; i<50; i++){
            int x1 = random.nextInt(WIDTH  - 4) + 2;
            int y1 = random.nextInt(HEIGHT - 4) + 2;
            int x2 = random.nextInt(WIDTH  - 2 - x1) + x1;
            int y2 = y1;
